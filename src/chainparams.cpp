...
{
    const char* pszTimestamp = "15/11/2020 En honor a Inti Sotelo y Bryan Pintado, víctimas de la represión policial durante la marcha nacional contra el régimen de Manuel Merino";
...
public:
    CMainParams() {
        ...
        //consensus.posLimit = uint256S("00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~uint256(0) >> 20
        //consensus.nPosTargetTimespan = 12 * 60 * 60; // SolD: 12 hours
        //consensus.nPosTargetTimespanV2 = 6000; // 
        consensus.nPowTargetTimespan = 24 * 60 * 60; // SolD: 1 day
        consensus.nPowTargetSpacing = 2.5 * 60; // SolD: 2.5 minutes
        ...
        genesis = CreateGenesisBlock(1605438000, 28917698, 0x1e0ffff0, 1, 50 * COIN);
        ...
        vSeeds.emplace_back("190.117.80.230");
        //vSeeds.emplace_back("dnsseed.sold.org");
       // vSeeds.emplace_back("dnsseed.solddot.io");

        // SolD addresses start with 'S'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,63);
        // SolD script addresses start with 's'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,55);
        // SolD private keys start with '7' or 'X'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,117);
        // SolD BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x66, 0x70, 0x7f, 0x67};
        // SolD BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x55, 0x7a, 0x8c, 0x70};

        // SolD BIP44 coin type is '5'
        nExtCoinType = 5; //
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlock(1605439800, 3861367235UL, 0x1e0ffff0, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        vSeeds.emplace_back("190.117.80.230"); // Just a static list of stable node(s), only supports x9
        //vSeeds.emplace_back("testnet-seed.solddot.io"); // Just a static list of stable node(s), only supports x9

        // Testnet SolD addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,125);
        // Testnet SolD script addresses start with '8' or '9'
        ...
        // Testnet SolD BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x66, 0x70, 0x7f, 0x0};
        // Testnet SolD BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x55, 0x7a, 0x8c, 0x0};

        // Testnet SolD BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
...
public:
    CDevNetParams() {
        ...
        pchMessageStart[3] = 0x4ce;
        nDefaultPort = 18805;
        nPruneAfterHeight = 1000;

        genesis = CreateGenesisBlock(1605441600, 1096447, 0x207fffff, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("soldevo.org",  "devnet-seed.soldevo.org"));
        ...
        // Testnet SolD BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x66, 0x70, 0x7f, 0x3};
        // Testnet SolD BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x55, 0x7a, 0x8c, 0x3};
...
public:
    CRegTestParams() {
        ...
        genesis = CreateGenesisBlock(1605443400, 1096447, 0x207fffff, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x000008ca1832a4baf228eb1553c03d3a2c8e02399550dd6ea8d65cec3ef23d2e"));
        assert(genesis.hashMerkleRoot == uint256S("0xe0028eb9648db56b1ac77cf090b99048a8007e2bb64b68f092c03c7f56a662c7"));
        ...
        // Regtest SolD addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,117);
        // Regtest SolD script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,85);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,102);
        // Regtest SolD BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x66, 0x70, 0x7f, 0x5};
        // Testnet SolD BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x55, 0x7a, 0x8c, 0x5};

        // Regtest SolD BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
