...
# sold_hash
RUN git clone https://github.com/soldpay/sold_hash
RUN cd sold_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} sold
RUN useradd -u ${USER_ID} -g sold -s /bin/bash -m -d /sold sold
...
RUN mkdir /sold-src && \
  ...
  chown $USER_ID:$GROUP_ID /sold-src && \
