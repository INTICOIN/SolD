...
const char * const BITCOIN_CONF_FILENAME = "sold.conf";
const char * const BITCOIN_PID_FILENAME = "soldd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "soldd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SolDCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SolDCore
    // Mac: ~/Library/Application Support/SolDCore
    // Unix: ~/.soldcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SolDCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SolDCore";
#else
    // Unix
    return pathRet / ".soldcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty sold.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure SolD Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("SolD Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The SolD Core developers";
    }
