...
public:
    ...
    // SolD Specific WalletInitInterface InitPrivateSendSettings
 ...
 * between soldd, and sold-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    if (mode == HMM_BITCOIN_QT)
        strUsage += HelpMessageOpt("-windowtitle=<name>", _("Sets a window title which is appended to \"SolD Core - \""));
    ...
    strUsage += HelpMessageOpt("-sporkaddr=<soldaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/soldpay/sold>";
    const std::string URL_WEBSITE = "<https://sold.org>";
...
/** Sanity checks
 *  Ensure that SolD Core is running in a usable environment with all
...
{
    // Make sure only a single SolD Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if SolD Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if SolD Core is started while in a temporary directory.\n",
        ...
        do {
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/soldpay/sold/pull/1817 and https://github.com/soldpay/sold/pull/1743
    ...
    // ********************************************************* Step 10c: schedule SolD-specific tasks
